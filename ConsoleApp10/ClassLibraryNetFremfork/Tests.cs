...
namespace ClassLibraryNetFremfork
{
   public class Tests
    {
        ...
        protected void Init(Action<object> notNullHandler, Action<bool> trueHandler)
        {
            ...
            // Настройте параметры API и номера телефонов в app. config
        ...
        private TelegramClient NewClient()
        {
            ...
            {
                throw new Exception($"Пожалуйста, добавьте свои настройки API в файл app. config. (More info: {MissingApiConfigurationException.InfoUrl})",
                                    ex);
            }
        }

        /// <summary>
        /// Чтение файла с настройками app. config
        /// </summary>
        private void GatherTestConfiguration()
        {
            string appConfigMsgWarning = "{0} не настроен в app. config! Некоторые тесты могут провалиться.";
        ...
        public virtual async Task AuthUser()
        {
            ...
            var code = CodeToAuthenticate; // вы также можете изменить код в отладчике

            if (String.IsNullOrWhiteSpace(code))
            {
                throw new Exception("CodeToAuthenticate пуст в файле app. config, заполните его кодом, который вы только что получили по SMS/Telegram");
            }
